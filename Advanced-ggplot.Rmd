...
dat1%>%ggplot(aes(x=var1,y=var2))+geom_point()+geom_smooth()+labs(x="Variable 1", y="Variable 2")
```
...
corr<-cor(dat3, use="complete.obs")
...
####PUT YOUR CODE HERE
dat4%>%ggplot(aes(x=var1, y=reorder(names,var1)))+geom_point(size=5)+theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), panel.grid.major.y = element_line(linetype = "dotted", color="red"))+labs(x="Variable1", y="")
```
...
####PUT YOUR CODE HERE
dat5%>%ggplot(aes(x=reorder(names,var1),y=var1))+geom_point()+geom_segment(aes(x=names,xend=names,y=0,yend=var1))+theme(axis.text.x = element_text(angle=90))+labs(x="", y="Variable1")
```
...
####PUT YOUR CODE HERE
library(ggalt)
ggplot(books_checked_out2,aes(y=reorder(Genre, Time1),x=Time1,xend=Time5))+geom_dumbbell(colour_x="purple",size_x=3, colour_xend="red", size_xend = 3)+labs(x="", y="Genre")
```
