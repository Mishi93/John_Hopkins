...
```{r,problem4}
boxplot(dat$faminc_new,xlab="Family income",main="Mehwish")
```
...
subset<-dat%>%filter(gender==1,region==1|region==2,marstat==1,newsint==1)
str(subset)
```
...
dat<-dat%>%mutate(tax_sales=recode(paste0(CC18_325d,CC18_325a), "22"=0, "12"=1, "21"=1, "11"=2))
head(dat$tax_sales,20)
```
...
```{r problem9}
dat%>%group_by(region)%>%summarize(average=mean(CC18_308a))
```
...
dat %>%
  ...
  summarize(
    mean = mean(pew_religimp),
    std_dev = sd(pew_religimp),
    median = median(pew_religimp)
  )
...
```{r problem11}
dat%>%summarize(mean=mean(faminc_new),std_dev=sd(faminc_new),median=median(faminc_new))%>%kable()
```
